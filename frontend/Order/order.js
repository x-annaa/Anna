...
async function loadCurrentRound(userId) {
  ...
  if (error) console.error("读取轮次状态失败", error);

  if (data) {
    window.currentRoundId = data.round_id;
    window.roundStartTime = data.start_time;
    window.completedOrders = data.completed_orders || 0;
    window.matchingEndTime = data.matching_end_time || null;
    window.matchingProductId = data.matching_product_id || null;

    // 如果有匹配倒计时
    if (window.matchingEndTime && window.matchingEndTime > Date.now()) {
      const delaySec = Math.ceil((window.matchingEndTime - Date.now()) / 1000);
      const { data: product } = await supabaseClient
        .from("products")
        .select("*")
        .eq("id", window.matchingProductId)
        .single();
      if (product) startMatchingCountdown(product, delaySec);
    } else if (window.matchingEndTime && window.matchingEndTime <= Date.now()) {
      // 匹配已结束但未生成订单
      finalizeMatchedOrderByServer(window.matchingProductId);
    }
